...
css_patch = """
        /* --- Accordion Sidebar CSS v6 (Auto-close & Upgrade Card) --- */
        ...
        .nav-menu { list-style: none; display: flex; flex-direction: column; gap: 8px; flex: 1; align-items: stretch; padding: 0; width: 100%; overflow-y: auto; overflow-x: hidden; margin-bottom: 20px; }
        ...
        .sidebar:hover .nav-item-header.active { color: var(--primary-purple); background: transparent; box-shadow: none; }
        .sidebar:not(:hover) .nav-item-header.active { background: linear-gradient(90deg, #6366f1 0%, #8b5cf6 100%); color: #fff; box-shadow: 0 4px 15px rgba(99, 102, 241, 0.25); }
        .sidebar:not(:hover) .nav-item-header.active i { color: #fff; }
        ...
        .upgrade-card { margin: auto 16px 20px 16px; border-radius: 12px; background: rgba(99, 102, 241, 0.08); border: 1px solid rgba(99, 102, 241, 0.15); display: flex; align-items: center; justify-content: center; padding: 12px 0; cursor: pointer; transition: all 0.3s; overflow: hidden; position: relative; flex-shrink: 0; text-decoration: none; }
        .upgrade-card:hover { background: rgba(99, 102, 241, 0.15); border-color: rgba(99, 102, 241, 0.3); transform: translateY(-2px); }
        .sidebar:hover .upgrade-card { justify-content: flex-start; padding: 12px 16px; }
        .upgrade-icon { font-size: 20px; color: var(--primary-purple); width: 24px; text-align: center; }
        .upgrade-content { display: none; white-space: nowrap; }
        .sidebar:hover .upgrade-content { display: block; margin-left: 16px; opacity: 0; animation: textFadeIn 0.3s 0.1s forwards; }
        .upgrade-content h4 { font-size: 13px; color: #fff; margin: 0 0 4px 0; font-weight: 600; }
        .upgrade-content p { font-size: 11px; color: var(--text-muted); margin: 0; }
"""
...
for filename in files:
    ...
    if '/* --- Accordion Sidebar CSS v5' in content:
    ...
    print(f"Patched {filename} with v6 CSS")
