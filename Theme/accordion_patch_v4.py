...
css_patch = """
        /* --- Accordion Sidebar CSS v4 (Exclusive Open + Perfect Center) --- */
        ...
        .logo-text { display: inline-block !important; font-size: 18px; font-weight: 600; margin-left: 12px; opacity: 0; max-width: 0; transition: opacity 0.2s ease, max-width 0.3s; white-space: nowrap; }
        .sidebar:hover .logo-text { opacity: 1; max-width: 200px; transition-delay: 0.1s; }
        ...
        .nav-item-header { display: flex; align-items: center; padding: 12px 15px; border-radius: 8px; color: var(--text-muted); cursor: pointer; transition: all 0.2s; margin: 0 16px; position: relative; overflow: hidden; }
        .nav-item-header:hover { background: var(--bg-card); color: var(--text-main); }
        .nav-item-header.active { color: var(--primary-purple); }
        
        .nav-header-left { display: flex; align-items: center; gap: 0; transition: gap 0.3s; }
        .sidebar:hover .nav-header-left { gap: 16px; }
        .nav-header-left i { font-size: 18px; min-width: 24px; text-align: center; }
        
        .nav-text { opacity: 0; max-width: 0; transition: opacity 0.2s ease, max-width 0.3s; white-space: nowrap; }
        .sidebar:hover .nav-text { opacity: 1; max-width: 150px; transition-delay: 0.1s; }
        ...
        .nav-item { display: flex; align-items: center; gap: 0; padding: 10px 15px; border-radius: 8px; color: var(--text-muted); text-decoration: none; font-size: 13px; font-weight: 500; transition: all 0.3s; white-space: nowrap; margin: 0 16px; overflow: hidden; }
        .sidebar:hover .nav-item { gap: 16px; }
...
# The exclusive open logic in JS - Note the double braces for literal JS object braces!
...
for filename in files:
    ...
    if '/* --- Accordion Sidebar CSS v3 (Perfect Centering & Smooth) --- */' in content:
        content = re.sub(r'/\* --- Accordion Sidebar CSS v3 .*?\*/.*?</style>', css_patch + '\n</style>', content, flags=re.DOTALL)
    else:
        # Fallback if somehow it didn't update properly
    ...
    print(f"Patched {filename} with v4 CSS")
